 ...
 */
@Entity
@Table(name = "MULTI_RELATIONAL_DISPLAY_FIELD_ENTITY")
...
public class MultiRelationalDisplayFieldEntity implements Serializable {
    ...
    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "MULTI_RELATIONAL_DISPLAY_FIELD_ENTITY_MANY_TO_MANY_DISPLAY_FIELD_ENTITY",
               joinColumns = @JoinColumn(name="multi_relational_display_field_entitys_id", referencedColumnName="ID"),
               inverseJoinColumns = @JoinColumn(name="many_to_many_display_field_entitys_id", referencedColumnName="ID"))
